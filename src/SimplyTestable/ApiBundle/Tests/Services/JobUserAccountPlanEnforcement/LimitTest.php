...
abstract class LimitTest extends ServiceTest {
    ...
    public function setUp() {
        ...
        for ($jobIndex = 0; $jobIndex <= $this->getJobTypeCreateLimit() - 1; $jobIndex++) {
            ...
            $job = $this->getJobService()->getById($this->createJobAndGetId(self::DEFAULT_CANONICAL_URL, null, $this->getJobType()));
    ...
    public function testLimitIsReached() {
        if ($this->isFullSiteLimitTest()) {
            $this->assertTrue($this->getJobUserAccountPlanEnforcementService()->isFullSiteJobLimitReachedForWebSite($this->getWebsite()));     
        } else {
            $this->assertTrue($this->getJobUserAccountPlanEnforcementService()->isSingleUrlLimitReachedForWebsite($this->getWebsite()));     
        }
    }
    ...
    protected function getExpectedJobTypeCreateLimit() {
            return $this->isFullSiteLimitTest() ? self::FULL_SITE_JOBS_PER_SITE_LIMIT : self::SINGLE_URL_JOBS_PER_URL_LIMIT;
    }
