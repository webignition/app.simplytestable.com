...
class LinkIntegrityPreProcessorTest extends BaseSimplyTestableTestCase {
    ...
    public function testHistoricTaskMaxAge() {
        $this->setHttpFixtures($this->getHttpFixtures($this->getFixturesDataPath(__FUNCTION__) . '/HttpResponses'));
        ...
        $linkIntegrityPreprocessor = $this->getTaskPreprocessorFactoryService()->getPreprocessor($tasks->first());

        $now = new \DateTime('-' . ($linkIntegrityPreprocessor->getMaxAge() + 1) . ' second');        
